...
//Consulta
$query = "DELETE FROM inmueble WHERE cod_inmueble=$cod_inmueble";
$result = mysqli_query($conn, $query) or die("Delete Query Failed");

if($result)
{	
	echo json_encode(array("message" => "Inmueble eliminado exitosamente", "status" => true));	
}
else
{	
	echo json_encode(array("message" => "Error al eliminar el inmueble", "status" => false));	
}
